                ...
                <table class="table table-striped table-borderless">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td><p class="mb-0"><span class="font-weight-bold slug">{{$berita->slug}}</span></p></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td><p class="mb-0">
                        <span class="font-weight-bold kategori">
                        @foreach ($berita->category as $item)
                          <label for="" class="badge badge-primary">{{$item}}</label>
                        @endforeach
                        </span></p></td>
                    </tr>
                  </tbody>
...
<script src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>  
  <script>
    $(document).ready(function() {
      let category = @json($berita->category);
      ...
      let dropdown = $("#category");
        ...
        $.getJSON(url, function (result) {
            $.each(result.data, function (key, entry) {
                if (category.includes(entry)) {
                  dropdown.append($('<option></option>').attr('selected','selected').attr('value', entry).text(eval(`entry`)));
                } else {
        ...
        if($('#category').val() != null){
          ...
          $.ajax({
            ...
            success: function(result){
              ...
              $('.kategori').text("");
              result.data.category.forEach(myFunction);
              function myFunction(item) {
                $('.kategori').append(`<label for="" class="badge badge-primary">${item}</label> `);
              }
